...
"$QUOTATOOL" -u nobody -i -q 3 -l 5 "$MNT" || fail "set inode limits failed"

# Create files as nobody — first 5 should work, 6th should fail
...
"$QUOTATOOL" -u nobody -i -q 0 -l 0 "$MNT" 2>/dev/null || true
